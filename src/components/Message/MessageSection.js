...
const MessageSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!nameError && !emailError && !messageError) {
      ...
      if (result.status === 200) alert('Message sent successfully');
      ...
      <div className="container">
        ...
        <div className="message-info">
          <form onSubmit={handleSubmit} noValidate>
            ...
            <label className={`${nameError ? 'error' : ''}`}>{`${nameError ? 'Your name needs to be submitted' : ''}`}</label>
            <input method="post" type="text" name="name" placeholder="Name" value={name} onChange={(e) => handleChange(e)} />

            <label className={`${emailError ? 'error' : ''}`}>{`${emailError ? 'Your email needs to be submitted' : ''}`}</label>
            <input method="post" type="email" name="email" placeholder="Email" value={email} onChange={(e) => handleChange(e)} />

            <label className={`${messageError ? 'error' : ''}`}>{`${messageError ? 'Your message needs to be submitted' : ''}`}</label>
